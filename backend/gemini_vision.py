...
# Module-level client — initialized once on first use (lazy, guarded by API key check)
_genai_model = None


def _get_model():
    """Return the cached Gemini GenerativeModel, initializing it on first call."""
    global _genai_model
...
def analyze_image_with_gemini(image_bytes: bytes, mime_type: str, question: str) -> str:
    ...
    Raises:
        ValueError: If the MIME type is unsupported or the image exceeds the size limit.
        RuntimeError: If the GEMINI_API_KEY is missing or the Gemini API call fails.
    """
